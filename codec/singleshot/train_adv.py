...
netD = p2p_networks.define_D(input_nc=3, ndf=64, netD='n_layers', n_layers_D=4, gpu_ids=[0])
...
def resume(model_name, index):
  names = ['encoder', 'binarizer', 'decoder', 'unet', 'd2', 'discriminator']
...
while True:
    ...
    for batch, (crops, ctx_frames, _) in enumerate(train_loader):
		...
		#rec2_losses.append((in_img - cum_output_d2).abs().mean())
                adv_lossD_inst = (criterionGAN(netD(output_d2.detach()), False) + criterionGAN(netD(in_img), True)) * 0.5
		...
		#cum_output_d2 += output_d2
                ...
                rec2_inst = (in_img - output_d2).abs().mean()
                adv_lossG_inst = criterionGAN(netD(output_d2), True)
                adv_lossesG.append(adv_lossG_inst)
                rec2_losses.append(rec2_inst)
                #rec2_losses.append((in_img - cum_output_d2).abs().mean())
            ...
            rec1_loss = sum(losses) / args.iterations
            rec2_loss = sum(rec2_losses) / args.iterations
            advG_loss = sum(adv_lossesG) / args.iterations
            loss = (rec1_loss + rec2_loss + lamb*advG_loss)*0.5
            ...
            print(
                    '[TRAIN] Iter[{}]; LR: {}; Rec1 Loss: {:.6f}; Rec2 Loss: {:.6f}; AdvG Loss: {:.6f}; AdvD Loss: {:.6f}; Backprop: {:.4f} sec; Batch: {:.4f} sec'.
		format(train_iter, 
		       scheduler.get_lr()[0], 
		       rec1_loss.item(),
		       rec2_loss.item(),
